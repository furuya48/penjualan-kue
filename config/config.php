...
function deleteProductAtCart($data)
{
    ...
    /*$cart = query("SELECT * FROM carts INNER JOIN products ON carts.product_id = products.id_product WHERE id_cart = $idProduct");*/
    /*$stok = $data["stock"];
    $barang = $data["banyak"];
    $stockProduk =  $stok + $barang;
    $queryProduct = "UPDATE products SET
                        stock = '$stockProduk'
                    WHERE id_product = $product_id
                    ";*/

    mysqli_query($conn, "DELETE FROM carts WHERE id_cart = $idProduct", /*$queryProduct*/);
    return mysqli_affected_rows($conn);
}
